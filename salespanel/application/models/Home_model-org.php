...
class Home_model extends CI_Model{
	...
	function fetch_sheets_materials($catid = NULL, $productid = NULL, $other = NULL)
    {
        if ($catid != NULL) {
            ...
            if ($productid != NULL and $other!= NULL) {
                  ...
                  $query = $this->db->query("SELECT * FROM `products` WHERE ProductID <> '$productid' AND CategoryID = '$catid' and Activate='Y' AND `ManufactureID` NOT LIKE '%-XS' order by sortBy asc");
             ...
             $query = $this->db->query("SELECT * FROM `products` WHERE ProductID = '$productid' and Activate='Y' AND `ManufactureID` NOT LIKE '%-XS' order by sortBy asc");
				...
				$query = $this->db->query("SELECT * FROM `products` WHERE CategoryID = '$catid' and Activate='Y' AND `ManufactureID` NOT LIKE '%-XS' order by sortBy asc");
	...
	function make_size_option($list, $field, $name, $selec=NULL){
					...
					if(count($list) == 1){
						...
						foreach($list as $a_row){
							  ...
							  if($a_row->$field==$selec){ $selected = 'selected="selected"';}
   ...
   function make_html_option($list, $field, $name, $selec = NULL) {
       ...
       $page = $this->input->post('page');
        ...
        if (count($list) == 1) {
            ...
            foreach ($list as $a_row) {
				...
				if($field=='SpecText7'){
					...
					if (preg_match("/Roll/i", $category)) { $category = 'Roll Labels';}
        ...
        if(isset($page) and $page != 'index'){ $option .= '<option value="" > Reset Selection </option>'; }   
	...
	function auto_search($page=NULL){
		...
		if($page!=''){ $condition = "AND Shape_upd NOT LIKE '%integrated%' ";}
	...
	function calculate_por($menucode, $table,$rolls,$labels){
        ...
        $query =    $this->db->query("Select $field from tbl_batch_roll INNER JOIN $table ON tbl_batch_roll.batch_id=$table.batch_id WHERE ManufactureID LIKE '".$menucode."' AND tbl_batch_roll.Rolls = $rolls ");
        $row = $query->row_array();	
        return $row[$field];
   }
 ...
 function min_roll_price($manufature1,$labels){
  ...
  function calculate_labels_sheets($manufature,$label){
   ...
   function calculate_rolls_qty($manufature,$roll){
		...
		if($Rolls==''){   $Rolls =  $this->min_qty_roll($manufature."1"); }
  ...
  function get_meter_against_diameter($code=NULL,$coreid=NULL,$diamter=NULL){
    ...
    function customize_product_name($data){
        ...
        if(isset($data['Source']) and ($data['Source'] == 'printing'  || $data['Source'] == 'LBA') and preg_match('/Roll Labels/is',$ProductBrand)){
            ...
            if($data['FinishType'] == 'No Finish'){ $labelsfinish = ' Label finish: None ';}



            else{  $labelsfinish = ' Label finish : '.$data['FinishType']; }
        ...
        if($printed=='Sample'){ $completeName = $completeName." - Sample ";}
    ...
    /***********label finder new changes*************/
	  ...
	  function genrate_shapes($list, $selected = NULL) {
        ...
        foreach ($list as $row) {
			...
			if(preg_match("/rectangle/is",$row->Shapes)){ $tooltip = 'RECTANGULAR LABELS'; }
			else if(preg_match("/triangle/is",$row->Shapes)){ $tooltip = 'TRIANGULAR LABELS'; }
			else if(preg_match("/tamper/is",$row->Shapes)){ $tooltip = 'TAMPER EVIDENT LABELS'; }
			else { $tooltip = strtoupper($row->Shapes).'  LABELS';}
  ...
  function customer_country_info(){
   ...
   function get_printed_attch($id){
	...
	function fetch_labelsby_category($level=NULL, $code=NULL){
		...
		if(isset($code) and $code!=''){ $condition = " parent_category LIKE '".$code."' AND ";}
	...
	function fetch_category_designs($code, $designid=NULL){
				...
				if($designid!=NULL){ $condition .= "  SUBSTRING(CategoryID, -4)  LIKE '".trim($designid)."' AND "; }
        ...
        function fetch_similar_category_designs($tcode, $code){
				...
				if(isset($row) and $row['SubCategoryID']!=''){ $code = $row['SubCategoryID']; }
...
//		$total_price = $printprice-$discount_price+$label_finish;
  ...
  function get_roll_qty($manufature1){
    ...
    function grouping_material_for_printed($condtion){
        $query=$this->db->query("SELECT * FROM `products` WHERE $condtion AND Activate='Y' group by LabelColor   order by priority asc ");
...
function rolls_calculation($die_across, $max_labels,  $total_labels, $rolls=NULL){
	...
	function desing_service_charges($design){
		...
		if($design == 1){ $price = 50;}



		else if($design == 2){ $price = 75;}



		else if($design == 3){ $price = 95;}



		else if($design == 4){ $price = 110;}



		else if($design == 5){ $price = 120;}
	...
	function get_printed_files($cartid, $ProductID){
	 ...
	 function make_option_with_tooltip($list, $field, $name, $selec = NULL) {
		...
		if($field == 'ColourMaterial_upd'){ $filter = 'material'; }



		else if($field == 'Adhesive'){ $filter = 'adhesive';}
		...
		if($field == 'Material1'){ $class = 'col-sm-6 col-xs-12'; $option = ''; }
	...
	*/	
	...
	function get_compatibility_text($method, $type, $product){
		...
		if(isset($row['description']) and $row['description']!=''){ return $row['description'];}else{ return '';}
	    ...
	    if(empty($check)){
		   ...
		   if($maxref==1){
			...
			$insert = array('jobno'=>$jobno,'ref'=>$maxref,'q1'=>1,'q2'=>1,'q3'=>1,'q4'=>1,'q5'=>1,'q6'=>1,'q7'=>1);   
			 ...
			 $q1 = ($pre_ref['q1']==0)?1:0; $q2 = ($pre_ref['q2']==0)?1:0;



			 $q3 = ($pre_ref['q3']==0)?1:0; $q4 = ($pre_ref['q4']==0)?1:0;



			 $q5 = ($pre_ref['q5']==0)?1:0; $q6 = ($pre_ref['q6']==0)?1:0;
...
///////////////////////********************************////////////////////////////////********************************
   ...
   function generate_category_shapes(){
    ...
    function grouping_product_material_colours($condtion){
        ...
        $query=$this->db->query("SELECT * FROM `products` WHERE $condtion AND Activate='Y' group by LabelColor order by `priority` desc ");
    ...
    function grouping_product_material_adhesive($condtion){
        ...
        $query=$this->db->query("SELECT * FROM `products` WHERE $condtion  AND Activate='Y' order by priority asc ");
