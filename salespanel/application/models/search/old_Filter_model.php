...
class Filter_model extends CI_Model{
    ...
    function labelfinder_data($condition,$sort=NULL,$start =NULL,$groupby=NULL){
        ...
        $qry =  "SELECT p.ManufactureID,SUBSTRING_INDEX( p.CategoryID, 'R', 1 ) as CategoryID,
						  p.ProductID,p.ManufactureID, p.ProductName,p.ProductBrand,p.ProductCategoryName,p.LabelsPerSheet,'' AS InnerHole,'' AS InnerLabel,
    ...
    function fetch_dies_data($condition, $sort=NULL,$limit=NULL){
        ...
        $qry2 = $this->db->query("SELECT c.CategoryID,c.specification1,c.specification2,c.specification3,c.pdfFile,
				c.wordFile, c.CategoryName, c.CategoryImage,c.LabelWidth,c.LabelHeight,c.Shape,c.labelCategory as ProductBrand 
				FROM category c 
    ...
    function distinct_material_paper($condtion){
        ...
        $query=$this->db->query("SELECT DISTINCT(material_type) AS Material FROM `products_colour` WHERE $condtion and 
		  material_type!='' order by material_type asc");
        return $query->result();
    }
    ...
    function distinct_material_color($condtion){
        ...
        $query=$this->db->query("SELECT DISTINCT(colour) AS Color FROM `products_colour` WHERE $condtion  
								        and colour!='' order by colour asc");
        return $query->result();
    }
