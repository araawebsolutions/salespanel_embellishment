...
class Filter_model extends CI_Model{
	...
	function get_min_width_height($field, $condition) {
	   ...
	   where $field != '' and (p.Activate ='Y' or p.Activate ='y' || p.displayin='both' || p.displayin='backoffice') AND $condition";
	...
	function labelsfinder_field_list($field, $condition){
					$qry =  "SELECT $field from products p where $field != '' and (p.Activate ='Y' or p.Activate ='y' || p.displayin='both' || p.displayin='backoffice') 
	...
	function labelfinder_counter($condition,$groupby=NULL){
			...
			if($groupby){
				...
				WHERE  ".$condition."  AND (p.Activate = 'Y' || p.displayin='both' || p.displayin='backoffice')";
			}else{
				$qry = "SELECT count(*) as total FROM products p WHERE ".$condition."  AND (p.Activate = 'Y' || p.displayin='both' || p.displayin='backoffice')";
			}	
			$query  = $this->db->query($qry);
			$result = $query->row_array();
			return $result['total'];		  
	...
	function fetch_dies_data($condition, $sort=NULL,$limit=NULL){
				...
				FROM category c 
				WHERE (c.CategoryActive LIKE 'Y' OR c.CategoryActive LIKE 'y' || c.displayin='both' || c.displayin='backoffice') AND ".$condition." GROUP BY c.CategoryID $sort $limit");
	...
	function grouping_compatiblity($compatible, $data){
		...
		function fetch_labelpersheet_data($condition, $type=NULL,$limit=NULL){
				if($type == "products")
				{
					...
					$start = 0;
					$qry2 =  $this->db->query("SELECT p.LabelsPerSheet as LabelPerDie,COUNT(*) as count FROM products p WHERE ".$condition." AND (p.Activate = 'Y' || p.displayin='both' || p.displayin='backoffice') GROUP BY LabelPerDie $sort limit $start ,12");
				...
				FROM category c WHERE (c.CategoryActive LIKE 'Y' OR c.CategoryActive LIKE 'y' || c.displayin='both' || c.displayin='backoffice') AND ".$condition." GROUP BY LabelPerDie $sort $limit");
