<div class="modal-content blue-background">
    <div class="modal-header checklist-header">
        <div class="col-md-12">
            <h4 class="modal-title checklist-title" id="myLargeModalLabel">Custom Dies Comments</h4>
    ...
    <div class="modal-body p-t-0">
        <div class="panel-body">
                        ...
                        <? foreach ($data as $row) {
                            ...
                            <tr class="">
                                ...
                                <td class="text-center orange-text">
                                    <a data-serial="<?= $row->serial ?>" data-id="<?= $row->ID ?>"
                                       class="deleter orange-text" style="cursor:pointer;">Remove</a>
                                </td>
                            </tr>
                        <?php } ?>
                    <? } else { ?>
            ...
            <div id="adder" style="display:none">
                <div>
                    <span class="return-title-texts">Add New Comment</span><br>
                    ...
                    <button id="save" data-serial="<?= $serial ?>"
                            class="btn btn-outline-info waves-light waves-effect p-6-10 m-r-10"
                            style="margin-left: 10px !important;">Save
                    </button>

                    <button id="show_foo" class="btn btn-outline-dark waves-light waves-effect btn-countinue m-r-10">
                        Cancel
                    </button>
                </div>
            </div>
            ...
            <span class="m-t-t-10 pull-right">
				<button type="button" id="foo" class="btn btn-outline-info waves-light waves-effect p-6-10 m-r-10"> Add Comment </button>
			</span>
...
<script>
    ...
    $('#save').click(function () { // the button - could be a class selector instead
        ...
        $.ajax({
            type: "post",
            url: mainUrl + "dies/dies/save_comment",
            cache: false,
            data: {comment: comment, serial: serial},
            dataType: 'html',
            success: function (data) {
                data = $.parseJSON(data);
                $('#edit_info_data').html(data.html);
                $('#comments_' + serial).html(data.count);
                $('#aa_loader').hide();
            ...
            .then((value) => {
                switch (value) {
                    case "yes":
                        ...
                        $.ajax({
                            type: "post",
                            url: mainUrl + "dies/dies/delete_comment",
                            cache: false,
