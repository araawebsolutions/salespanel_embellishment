  ...
  <div class="container">
   ...
   <form id="artwork_approval" method="post">
    ...
    <div class="thumbnail prMatDC">
      ...
      <div class="col-md-12">
        <div class="titleX"> <b>Approbation des œuvres</b> </div>
        <p id="decline-section" style="display:none">Vous avez sélectionné une version précédemment refusée, cliquez sur Continuer pour continuer.</p>
        <p id="qa-section-header">Avant d'approuver cette illustration pour l'impression, il est important que yoo vérifie ce qui suit:</p>
      ...
      <p class="text-justify" id="qa-section-footer">Une fois que vous avez vérifié et approuvé votre conception d'étiquette, utilisez le bouton ci-dessous pour répéter le processus pour d'autres modèles et pour procéder à la production de vos étiquettes imprimées. Si le ou les projets nécessitent une modification, utilisez les cases des commentaires à droite du formulaire, à côté des cases à cocher, pour saisir les détails de vos besoins et retourner à l'équipe de service à la clientèle pour l'action en utilisant le bouton ci-dessous.</p>
        ...
        <div class="">
          <div class="row">
            ...
            <div class="col-md-4  col-sm-4 col-xs-12 text-center"> 
            	<strong class="cBlue artworkno" id="artworkno"> 1 </strong> sur <strong class="cBlue"> <?=$total?>  </strong> 
            </div>
            <div class="col-md-4 col-sm-4 col-xs-12">
            
             <button type="button"  id="pleasewaitbutton" class="btn btn-block orange" disabled style="display:none">
                 <img onerror='imgError(this);' id="world_loader_icon" src="<?=Assets?>images/ring.gif" /> S'il vous plaît, attendez.... <i class="fa fa-arrow-circle-right"></i> 
             </button>
                              
              <button type="button" class=" btn btn-block orange pull-right formsubmit" name="feedback[type]" value="approve" id="approver"> 
              Procéder <strong class="artworkno" > 1 </strong> sur <strong class=""> <?=$total?>  </strong>  </button>
              
              <button class="btn btn-block orange formsubmit" type="button" name="feedback[type]" value="amend" style="display:none" id="amender"> Procéder <strong class="artworkno" > 1 </strong> sur <strong class=""> <?=$total?>  </strong> 
              </button>
            </div>
          </div>
        </div>
...
$(document).on("click", ".ReviseSoftproofsList", function(e) {
	...
	if(checkddecline>1){ title = title+' - <b style="color:red">declin</b>'; }
...
$(document).on("click", ".formsubmit", function(e) { 
		...
		if(flag == 'amendments'){
			swal('Veuillez entrer les modifications requises pour continuer.');	
			return false;
		}
			...
			if(artworkrefval==1){
			  var poptext = "veuillez confirmer pour continuer";
			}else{
			var poptext = (display=="block")?"Vous approuvez une version precedemment refusee de lepreuve-écran. Veuillez cliquer sur le bouton de confirmation pour continuer, ou annuler pour retourner a lillustration.":"Poursuivre avec le fichier revise?";
			}
		...
		function(isConfirm) {
		  if (isConfirm){	
			...
			data.push({name: 'feedback[type]', value: type});
				...
				swal('Veuillez compléter la liste de contrôle pour continuer.');
		}
