...
class cart extends CI_Controller {
	...
	function __construct(){
		...
		// $this->load->model('orderAttachment');
    ...
    public function makePlainData($records,$data){
	    ...
	    elseif($records->ProductBrand == 'Roll Labels'){
           // print_r($this->myPriceModel->getPrice($data['orderNumber'],$data['manfactureId'],$data));exit;
            $data['price'] = $this->myPriceModel->getPrice($data['serialNumber'],$data['orderNumber'],$data['manfactureId'],$data)['totalPrice'];
            $dataArray = $this->makePlainArray($records,$data);
            $data['wound'] = ($records->Wound == null)?'':$records->Wound;
	...
	//	print_r($calculation); die();
        ...
        if($record->ProductBrand == 'Integrated Labels'){
            $originalQty  = (preg_match('/250 Sheet Dispenser Packs/is',$record->ProductName))?250:1000;
            
        }else{
            $originalQty = $data['statics']->totalLabels;
        }
			...
			if($calculation['totalPrice']==""){
             ...
             if($record->ProductBrand == 'Roll Labels'){
                ...
                $totallabels =  $record->LabelsPerRoll * $data['qty'];
        ...
        $printedArray =  array(
            ...
            'Print_Design'=>$record->Print_Design,
    ...
    function get_db_column($table, $column, $key, $value){
	 ...
	 public function paymentPage(){
        ...
        if($_POST){
            ...
            if($this->form_validation->run() != false){
                ...
                if(isset($paymentway) && $paymentway == "creditCard" and $sample!='sample'){
					$data['errortype'] = 'payment';
                    $record = $this->worldPayForCart();
                   if(isset($record['order_no'])){
                        $d = $data['order'] = $this->orderModal->getOrder($record['order_no']);
                        $data['orderDetails'] = $this->orderModal->getOrderDetail($record['order_no']);
                    ...
                    //$this->sendEmailForPayPol($this->session->userdata('OrderNumber'));
                    $d = $data['order'] = $this->orderModal->getOrder($this->session->userdata('OrderNumber'));
                    $data['orderDetails'] = $this->orderModal->getOrderDetail($this->session->userdata('OrderNumber'));
                    $data['status'] = $this->orderModal->statusDropDown($d[0]->OrderStatus,$d[0]->PaymentMethods);
                    $data['main_content']  ='order_quotation/checkout/confirm_my_order';
                    $this->load->view('page',$data);
                    return true;
		...
		if(count($cal) > 0){
		  ...
		  $data['records'] = $cal;//$this->productCalculation($records);
		  $ii = count($cal)-1;
		  $data['digitalis'] = $this->cartModal->digitalPrintingProcess($records);
...
//        $record['price'] = $cal[$ii]->total;
    ...
    public function sendEmailForPayPol($orderNumber){
		...
		$mailfrom  ='customercare@aalabels.com'; $mailname="AAlabels";
...
//
//                    redirect(main_url.'cart/Cart/worldpaycallback/');
//
...
//                }
                if(isset($response['paymentStatus']) && $response['paymentStatus'] == 'SUCCESS') {
                    ...
                    $this->db->update("orders",array('OrderStatus'=>$OrderStatus,'YourRef'=>$response['orderCode'],'Payment'=>1));
                    $data['order_no'] =  $response['customerOrderCode'];
                    return $data;
                }
                else{
                    $data['error']  = $response['paymentStatusReason'];
                    return $data;
                }
    ...
    private function save_credit_card($token){
        ...
        if(isset($retain_cards) and $retain_cards==1){ return;}
        ...
        if($count == 0){
            ...
            require_once(APPPATH.'libraries/worldpay.php');
            $worldpay = new Worldpay(WP_SERVICE_KEY);
            if(SITE_MODE!='live'){ $worldpay->disableSSLCheck(true); } //remove this line after live
            try {
