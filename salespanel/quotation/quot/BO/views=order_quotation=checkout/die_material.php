...
if ($quotationDetail->regmark != 'Y') {
    ...
    <tr <? if ($quotationDetail->Printing != 'Y') { ?> style="display: none" <? } ?>>
        ...
        <td colspan="1">
            ...
            <div class="btn-span" id="artwork_section<?= $key ?>">
                ...
                <select id="digital<?= $key ?>" class="form-control no-padding m-10" name="digital">
                    <?php foreach ($digitalis as $digital) {
                        if ($digital->type == $digitalCheck || $digital->type == 'both') { ?>
                            ...
                            <option value="<?= $digital->name ?>" <?php if ($digital->name == $quotationDetail->Print_Type) {  echo 'selected';
                            } ?>><?= $digital->name ?></option>
                        <?php } } ?>
              ...
              <!--<span class="no-padding m-10">
                <input type="checkbox" id="pressProf<?= $key ?>"
                       <?php if ($quotationDetail->pressproof == 1){ ?>checked
                       <?php } ?>name="pressprof" value="1"> <p style="font-size: 11px;color: #666;">Pressproof</p> </span>-->
              <?php } ?>
        ...
        <?=$symbol?><?=number_format(5.32 * $exchange_rate, 2,'.','')?><br>Per Design
      ...
      if($quotationDetail->Free > 1){
        ...
        <input class="form-control input-number text-center allownumeric" <?php if ($quotationDetail->Printing == 'Y') { ?> readonly="readonly" <?php } ?> type="hidden" id="design<?= $key ?>" value="<?php echo $qt ?>">
