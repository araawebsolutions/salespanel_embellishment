  ...
  class Datatables
  {
      ...
      for($i = 0; $i < intval($this->ci->input->post('iSortingCols')); $i++){
         // print_r($mColArray);exit;
          if(isset($mColArray[intval($this->ci->input->post('iSortCol_' . $i))]) ){
              //echo 'h';exit;
              $this->ci->db->order_by($columns[intval($this->ci->input->post('iSortCol_' . $i))], $this->ci->input->post('sSortDir_' . $i));
          }
      }
    ...
    */
    protected function get_filtering()
    {
      ...
      $sWhere = '';
      $sSearch = trim($this->ci->input->post('sSearch'));
      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray)-1; $i++){
            ...
            if($this->ci->input->post('bSearchable_' . $i) == 'true')
                $sWhere .= $this->select[$columns[$i]] . " LIKE '%" . $sSearch . "%' OR ";
        }
...
public function getproduction($rows,$count,$charset=NULL,$condition=null)    {
        $aaData = array();
        $iTotal = $count;
            ...
            foreach($this->add_columns as $field => $val)
                ...
                foreach($this->edit_columns as $modkey => $modval)
                    ...
                    $aaData[$row_key] = array_diff_key($aaData[$row_key],
                        ($this->check_mDataprop())? $this->unset_columns : array_intersect($this->columns, $this->unset_columns));
                    if(!$this->check_mDataprop())
                        $aaData[$row_key] = array_values($aaData[$row_key]);
        }
        $sColumns = array_diff($this->columns, $this->unset_columns);
        $sColumns = array_merge_recursive($sColumns, array_keys($this->add_columns));
        $sOutput = array('sEcho'=> intval($this->ci->input->post('sEcho')),        'iTotalRecords'        => $iTotal,        'iTotalDisplayRecords' => $iFilteredTotal,        'aaData'               => $aaData,        'sColumns'             => implode(',', $sColumns) );
        if($condition == 'dashboard'){
            return $sOutput['iTotalRecords'];
        }else{
            if(strtolower($charset) == 'utf-8')
                return json_encode($sOutput);
            else
...
/* End of file DataTablesController.php */
/* Location: ./application/libraries/DataTablesController.php */