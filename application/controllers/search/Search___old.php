...
//        }
    ...
    function get_nearest_category($catid, $p_id, $pageName, $regmark)
    {
        ...
        if ($row['total'] == 1) {
            ...
            $query = $this->db->query("select CategoryID from category WHERE Shape LIKE '" . $row['Shape'] . "' 
				...
				AND CategoryActive='Y' AND (displayin='both' OR displayin='backoffice') ORDER BY Width ASC  LIMIT 1");
            $row = $query->row_array();
    ...
    function integrated_detail($cat = null)
    {
		...
		AND CategoryActive='Y' and (displayin='both' OR displayin='backoffice') AND `specification3` = 'Integrated'  LIMIT 5");
        $result = $query->result();
    ...
    function integrated($cat = NULL, $compatible = NULL)
    {
        ...
        if (isset($row['SubCategoryID']) and $row['SubCategoryID'] != '') {
            ...
            $condition = " p.CategoryID IN (" . $rel_prd_string . ") AND CategoryActive='Y' and (displayin='both' OR displayin='backoffice')";
            if ($cat == 'T813') {
                $condition = " p.CategoryID IN (" . $rel_prd_string . ") ";
            }
        } else {
            $condition = " p.ProductBrand LIKE 'Integrated Labels' AND CategoryActive='Y' AND (displayin='both' OR displayin='backoffice') AND c.Shape NOT LIKE 'Full Sheet Integrated' ";
...
//    }
    ...
    public function getSearchResults()
    {
        ...
        if ($category) {
            $query .= $condition = " p.ProductBrand LIKE '" . $brand . "' AND CategoryActive='Y' and ( displayin='both' OR displayin='backoffice')";
...
*/
    ...
    function grouped_product_info()
    {
        ...
        $Lwidth = $this->input->post('width', true);
        $Lheight = $this->input->post('height', true);
        $condition = " Activate='Y' and (displayin='backoffice' OR displayin='both') AND LabelFinish LIKE '" . $finish . "' AND ColourMaterial LIKE '" .
            $material . "' 
				AND LabelColor LIKE '" . $colour . "' AND CategoryID LIKE '" . $categoryid . "'";
        $adh_list = $this->db->query("SELECT Adhesive FROM `products` WHERE $condition 
				GROUP BY Adhesive order by ProductID asc ")->result_array();
