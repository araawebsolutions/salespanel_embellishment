...
class CustomDieModal extends CI_Model
{
                ...
                if($width !=0){
                    ...
                    if($actualPrice !=0){
						...
						if($_REQUEST['shapes']!="Irregular"){
							...
							$data['actualPrice'] = '0.00';
							$data['price'] = '0.00';
							$data['discount'] = '0.00';
							$data['discountVal'] ='0.00';
						}
    ...
    public function getNearestMatch($width,$height,$format,$shape,$check=null){
		...
		$result = $this->db->select("width,Height,(Width + Height) as total,  ABS( ".$cal." - (Width + Height) ) AS distance,CategoryImage as code FROM `category` where  labelCategory = '".$format."' AND CategoryActive = 'Y' AND Shape = '".$shape."' AND LabelAcross = ".$across."   ORDER BY distance ASC LIMIT 1")->get();
		...
		if($result->num_rows()==0){
			$result = $this->db->select("CategoryImage as code FROM `category` where  labelCategory = '".$format."' AND CategoryActive = 'Y' AND Shape = '".$shape."' AND LabelAcross = ".$across." ORDER BY DieCode DESC LIMIT 1 ")->get();
		}
		
		if($result->num_rows()==0){
			$result = $this->db->select("CategoryImage as code FROM `category` where  labelCategory = '".$format."' AND CategoryActive = 'Y' AND LabelAcross = ".$across." ORDER BY DieCode DESC LIMIT 1 ")->get();
		}
		
		if($result->num_rows()==0){
         $result = $this->db->select("width,Height,(Width + Height) as total,  ABS( ".$cal." - (Width + Height) ) AS distance,CategoryImage as code FROM `category` where  labelCategory = '".$format."' AND CategoryActive = 'Y' AND Shape = '".$shape."' ORDER BY distance ASC LIMIT 1")->get();
        }
	
		

		$result2 = $result->row_array();
        ...
        if($result->num_rows()){
            ...
            if($check === 'true' /*&& ($result2['code'] == $dieCode[0]->die_code)*/){
				
				$ret = $this->removeExtra($result2);
                return $ret;
            }else{
            ...
            //echo $diff;exit;
            $records = $this->db->select("(Width + Height) as total,DieCode as code FROM `category` where  labelCategory = '".$format."' AND CategoryActive = 'Y' AND Shape = '".$shape."'  HAVING total <=> ".$cal."  ORDER BY total ")->get()->result_array();
            $closest = null;
    ...
    public function removeExtra($record){
        ...
        $value = explode('.',(isset($record['code']))?$record['code']:$record['code'])[0];
    ...
    public function cancelCart(){
        ...
        $this->session->set_flashdata('cut_data','cust');
        return true;
    }
