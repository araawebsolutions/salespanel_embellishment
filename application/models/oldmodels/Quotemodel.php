...
class Quotemodel extends CI_Model {
	...
	public function addToCart($ProductID,$ManufactureID,$Quantity,$cusLbl=null,$conidition = null)
	{
		...
		//echo "reg:$regmark";exit;
				$prodinfo = $this->orderModel->getproductdetail($ProductID);
				$is_custom = 'No';
				$labelproll=0;
				$totallabels = 0;
			...
			if(preg_match('/Roll Labels/is',$prodinfo['ProductBrand']))
			  {
				  ...
				  $totallabels = $labelproll*$Quantity;
			  }
              ...
              else if(preg_match('/SRA3 Label/',$prodinfo['ProductBrand']))
			  {
	           ...
	           // echo $ManufactureID.' q='.$Quantity.' l='.$cusLbl.' qq='.$Quantity;
				...
				echo "ManufactureID: $ManufactureID<br>";
				echo "cusLbl: $cusLbl<br>";
				echo "custom_price: $custom_price<br>";
				echo "regmark: $regmark";*/
				
				if($regmark == "yes")
				{
					    $collection['labels'] 	  = $totallabels; 
						$collection['manufature'] = $ManufactureID;
						$collection['finish']     = 'No Finish';
						$collection['rolls']      = $Quantity;
						$collection['printing']   = 'Mono';
								   
						$price_res = $this->calculate_printing_price($collection);
						$regmarkprice = $price_res['final_price']; 
					    $custom_price = $regmarkprice;
...
if($ProductID==0){
				...
				if(isset($regmark) and $regmark == "yes"){
					$A4Printing = array( 'Printing'=>'Y',
										 ...
										 'Free'=>1,
										 'Print_Qty'=>1,
										 'orignalQty'=>$labelproll*$Quantity,
										 ...
										 'regmark'=>'Y',
										 'orientation'=>1);
					$quotationData = array_merge($quotationData,$A4Printing);
				}
        ...
        if(preg_match('/Roll Labels/is',$ProductBrand) && $printing=="Y"){
            ...
            $UnitPrice = number_format(round($unitPrice,3),3,'.','');
			 ...
			 $regmark = $this->input->post('regmark');
			  ...
			  if(isset($regmark) and $regmark == "Y")
			  {
				 
				 	//$custom_price = $custom_price + REGMARKPRICE;
					$collection['labels'] = $LabelsPerSheet*$qty; 
					$collection['manufature'] = $ManufactureID;
					$collection['finish'] = 'No Finish';
		  ...
		  $print_type = '';
		   ...
		   $IncVat = round($cartdata->TotalPrice * vat_rate,2);
			...
			if($cartdata->source=='printing' and preg_match('/Roll Labels/is',$prodinfo['ProductBrand'])){
					...
					$labeltype = $this->orderModel->get_printing_service_name($cartdata->Print_Type);
		   ...
		   if(preg_match('/Integrated Labels/is',$prodinfo['ProductBrand']) || $cartdata->Printing=='Y'){
					if($cartdata->OrderData=='Black' || $cartdata->OrderData=='Printed' || $cartdata->Printing=='Y'){
								...
								if($query['total'] > 0 || $cartdata->regmark == "Y"){
									...
									{
										$brand = $this->orderModel->make_productBrand_condtion($prodinfo['ProductBrand']);
										...
										$fakearray['QuotationNumber'] = $QuotationNo;
										$fakearray['Serial'] = $quote_serail;
										...
										*/
										...
										$this->db->insert('quotation_attachments_integrated',$fakearray);
										/*CO,sp,ca,pf = 1;
										status = 70
										action = 0;
										file = .pdf;
	 ...
	 function is_quotation_integrated($quoteNo){
	  ...
	  //************************************//**************************************//
        ...
        public function saveorder($quoteID,$paymentmethod)
        {
		 ...
		 $OrdNo = $this->orderModel->getOrderNum();
		if($OrdWebsite=='123')
		{
			$orderNo = '123-'.$OrdNo;
		}
			...
			$orderNo = 'AA'.$OrdNo;
             ...
             foreach($order_detail as $order_detail){
					...
					$Print_Type = $order_detail->Print_Type;
					 ...
					 $prodinfo = $this->orderModel->getproductdetail($order_detail->ProductID);
				  ...
				  if( $order_detail->Printing=='Y'){
								...
								if(count($artowrks) > 0){
										foreach($artowrks  as $int_row){
											...
											$brand = $this->orderModel->make_productBrand_condtion($brand['ProductBrand']);
...
if(substr($ManufactureID,-2,2)=='XS'){
			...
			$ProdName = $this->orderModel->customize_product_name('',$prodinfo['ProductCategoryName'],'',$prodinfo['LabelsPerSheet'],$prodinfo['ReOrderCode'],$ManufactureID,$prodinfo['ProductBrand'],$Wound,$printtype);
  ...
  function wtp_discount_applied_on_order(){
   ...
   function check_wtp_offer_voucher(){
		...
		$query = $this->db->query(" SELECT count(temporaryshoppingbasket.ProductID) AS total from products INNER JOIN temporaryshoppingbasket ON 
		      						products.ProductID=temporaryshoppingbasket.ProductID WHERE ManufactureID LIKE '%WTP' AND $session AND 
			   						temporaryshoppingbasket.Quantity >= 10000 AND  (ProductBrand NOT LIKE 'Roll Labels' AND ProductBrand NOT LIKE 'SRA3 Label' AND ProductBrand NOT LIKE 'A3 Label' AND ProductBrand NOT LIKE 'Integrated Labels')");
		$row = $query->row_array();	
	...
	$query = $this->db->query(" SELECT SUM(TotalPrice) AS total from products INNER JOIN temporaryshoppingbasket ON 
		      						products.ProductID=temporaryshoppingbasket.ProductID WHERE ManufactureID LIKE '%WTP' AND $session AND 
			   						temporaryshoppingbasket.Quantity >= 10000 AND  (ProductBrand NOT LIKE 'Roll Labels' AND ProductBrand NOT LIKE 'SRA3 Label' AND ProductBrand NOT LIKE 'A3 Label')");
	$row = $query->row_array();	
...
public function check_product_extra_detail($ManufactureID,$ProductBrand=NULL){
 ...
 /*******  Roll Printing Formula *******/
	 ...
	 function calculate_printing_price($collection = array()){
		// print_r($collection);
		  ...
		  if(isset($labels) and  $labels > 0){
				...
				/**********************************************************************/
				 ...
				 //$total_price = $plainprice+$printprice+$label_finish;
	   ...
	   function calculate_print_material__oooolllllll($ManufactureID, $Labels, $finish, $rolls){
...
function labels_discount_breaks($labels){
   ...
   public function get_custom_prize($mat,$qty,$format){
	...
	public function fetch_product_name($data){
	  ...
	  $type = $this->orderModel->make_productBrand_condtion($data['ProductBrand']);	
	  if($type=="Rolls"){
			 ...
			 // $printed= $data['OrderData'];
			  $printed ='';
					   ...
					   if(isset($data['Source']) and $data['Source'] == 'Y' and preg_match('/Roll Labels/is',$ProductBrand)){
						 ...
						 $labeltype  = $this->orderModel->get_db_column('digital_printing_process', 'name_fr', 'name', $data['Print_Type']);
	                  ...
	                  if($custom=='Yes'){
						 ...
						 $completeName =  $productname[0].$LabelsPerRoll." ".lang('placeholder_labels_per_roll').", ".$wound_opt." - ".$productname[1]; 
								  $diamter =  $this->orderModel->calculate_rolls_diamter($manuid,$LabelsPerRoll);
	...
	function fetch_custom_die_association($id){
	  return $this->db->query("select * from flexible_dies_mat where OID = $id order by material asc")->result();
	}
						...
						foreach($prod as $pr)
						{
							...
							{	
								...
								if($printing == "Y")
								{
									...
									if($row->pressproof == 1)
									{
										$pressproof_charges = 50;
										$total_price = $total_price + $pressproof_charges;
									}
								}
								else
								{
									$total_price = $this->calclateprice($pr['ManufactureID'],$row->Quantity,$row->LabelsPerRoll);
	...
	function split_trade_order($orderNum)
	{
		...
		if($user_domain)
		{
			$orderDetails = $this->orderModel->OrderInfo($orderNum);
			$orderDetails = $orderDetails[0];
			$orderLines = $this->orderModel->OrderDetails($orderNum);
			...
			{ 
				...
				foreach($cart_detail as $row)
				{
					...
					foreach($prod as $pr)
					{
						...
						{	
							$printing = $row->Printing;
							if($printing == "Y" and $row->regmark != "Y")
							{
								$qty = $row->Quantity;
								$min_qty = $this->quoteModel->min_qty_roll($pr['CategoryID']);
	  ...
	  $regmark       = $this->input->post('regmark');  
		  ...
		  if(preg_match('/Roll Labels/is',$ProductBrand) && $printing=="Y"){
				 ...
				 if($regmark == "Y")
				{
				        ...
				        $min_qty = $this->get_roll_qty($ManufactureID);
				        $response = $this->rolls_calculation($min_qty,$prid['LabelsPerSheet'],$lpr*$qty);
						...
						$collection['finish']     = 'No Finish';
						$collection['rolls']      =  $response['rolls']; 
						$collection['printing']   = 'Monochrome – Black Only';
					//	echo '<pre>'; print_r($collection);
						$price_res = $this->calculate_printing_price($collection);
						$regmarkprice = $price_res['final_price']; 
						$custom_price = $regmarkprice;
						
					//	echo '<pre>'; print_r($price_res);
				}
