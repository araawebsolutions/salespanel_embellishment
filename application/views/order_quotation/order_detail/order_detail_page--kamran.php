  ...
  <div class="container-fluid">
              ...
              <div class="col-md-4" style="display: flex">
                <div class="card enquiry-card enquiry-card-bix-box-second" style="width: 100%">
                  ...
                  <div class="card-body"> Order:
                    ...
                    Courier & Tracking No: <?php echo $order->OrderDeliveryCourier;?> <?php echo $order->DeliveryTrackingNumber; ?> <br>
                    Dispatch Date:
                  ...
                  <tbody style="color: #666;">
                    ...
                    <tr>
                      ...
                      <td class="padding-6 icon-tablee"><i class="fa fa-trash-o bt-delete"
                                                           ...
                                                           id="deletenode1"></i> <i class="fa fa-floppy-o bt-save" onclick="updateOrderNewLine(<?= $key ?>,<?= $detail->SerialNumber ?>,<?= $detail->UserID ?>)"></i></td>
                    </tr>
                    <?php }
                    ...
                    <tr>
                      ...
                      <td class="text-center"><i class="mdi "></i>
                        <?= $this->user_model->get_mat_name($rowp->material); ?></td>
                      <td class="text-center" id="labels0">-</td>
                      <td class="text-center"><?= $rowp->qty ?></td>
                      <td class="text-center"><?=$symbol?>
                        <? echo number_format($materialprice * $exchange_rate, 2); ?>
                        <?php $totalPrice +=  number_format($materialprice * $exchange_rate, 2);?></td>
                      <td></td>
                    </tr>
                    <?php }?>
                    <?php } }?>
    ...
    <?
       if(preg_match('/Roll Labels/is',$detail->ProductBrand) && $detail->Printing=="Y" and $detail->regmark != 'Y'){
            ...
            if($detail->Wound=='Y' || $detail->Wound=='Inside'){ $wound_opt ='Inside Wound';}else{ $wound_opt ='Outside Wound';}
        	 ...
        	 if($detail->FinishType == 'No Finish'){ $labelsfinish = ' With Label finish: None ';}
        	 else{  $labelsfinish = ' With Label finish : '.$detail->FinishType; }
     ...
     ?>                                 
                      ...
                      <td class="text-center"
                                                        id="labels<?= $key ?>"><?= ($detail->sample == 'sample' || $detail->sample == 'Sample') ? 0.00 : $detail->labels ?></td>
                      ...
                      <td class="padding-6 icon-tablee"><?php
                        ...
                        <i class="fa fa-floppy-o bt-save" id="update_one<?= $key ?>" 
      ...
      if($detail_view['prompt']=='yes'){?>
                    ...
                    <tr id="order_note_line<?= $key ?>" <?php if ($detail->Product_detail == null) { ?> style="display: none;" <?php } ?>>
                      ...
                      <td class="padding-6 icon-tablee"><i class="fa fa-trash-o bt-delete"
                                                           onclick="deleteNoteForOrder(<?= $detail->SerialNumber ?>)"> </i> <i class="fa fa-floppy-o bt-save"
                      ...
                      <td><div class="btn-span"> <span class="labels-form" style="margin-right: 15px;">
                          ...
                          <label class="select" style="margin-bottom: 0px !important;">
                            <select id="digital<?= $key ?>" class="form-control no-padding" name="digital"  onchange="insertLog(this.value,<?=$detail->SerialNumber?>,'Print_Type')">
                              ...
                              <?php } else{?>
                              <option value="<?= $digital->name ?>" <?php if ($digital->name == $detail->Print_Type || $digital->Print_Type == $detail->Print_Type) { echo 'selected'; } ?>>
                              <?= $digital->name; ?>
                              </option>
                              <?php } ?>
                              <?php }
                    ...
                    <tr>
                      <td class="text-center"><?php if ($order->editing == "no") { ?>
                        <b> <span style="color:green;font-size:12px; ">Product Note</span></b>
                        <?php  } ?></td>
                      <td><?php if ($order->editing == "no") { ?>
                        <b><span style="color:green;font-size:12px ">
        ...
        class="btn btn-success waves-light waves-effect">Finish Edit Order</button>
                ...
                <div class="col-md-6 pull-right">
                  <table class="table table-bordered quote-price-details">
                    ...
                    <tr>
                      <td>Delivery Total: <?php echo $order->OrderShippingAmount ?></td>
                    ...
                    <tr>
                      <td>VAT Exempt:</td>
                      <td>-
                        <? $vatvalue = ($totalPrice * vat_rate) - $totalPrice;     
                    ...
                    <?php
                        $refnd = number_format($paymentReceived - $finalAmount, 2);
                               ...
                               $fn = number_format($myfinalAmount * $order->exchange_rate, 2, '.', '');
                                        ?>
                    <?php if ($paymentReceived < $finalAmount && $fn != '12') { ?>
                    <tr>
                    ...
                    <tr>
                      <td class="labels-form"><label class="select">
                          ...
                          <i></i> </label></td>
                      <td><button type="button" onclick="printingorderinvoice('<?= $order->OrderNumber ?>');" class="btn btn-info waves-light waves-effect"> Print Invoice </button></td>
                    </tr>
                      </tbody>
      ...
      <div class="modal-body p-t-0">
        <div class="panel-body">
          ...
          <button id="up_nt" type="button" style="display: none;" onclick="updateNote()"
                                class="btn btn-outline-dark waves-light waves-effect btn-countinue btn-print1" style="margin-right: 10px;">
          Update Note
          </button>
          </span> </div>
      </div>
