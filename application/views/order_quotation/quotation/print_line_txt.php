<td class="invoicetable_description_loop invoicetable_tabel_border" style=" <?=(isset($print_style) and $print_style!='')?$print_style:''?>">
    <? $type = $typeshow = $desntype = $this->quotationModal->get_printing_service_name($AccountDetail->Print_Type); ?>
    ...
    if(isset($showartowrks) and $showartowrks =='Yes' and $AccountDetail->Printing=='Y'){
        $artowrks = $this->quotationModal->get_printed_files($AccountDetail->SerialNumber); ?>
        <? if(count($artowrks) > 0){
            ?>
            <div>
            <hr />
