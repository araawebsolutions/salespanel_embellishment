...
var datatable_count = 1;
 ...
 $(document).on("click", ".mover", function(e) {
      var order = $(this).attr('data-id');
      is_all_templates(order);
  });



function is_all_templates(order){

    $.ajax({
        async:true,
        type: "post",
        url: mainUrl+"Artworks/is_upload_template",
        cache: false,
        data:{order:order},
        //dataType: 'html',
        success: function(data){
            ...
            if(data.t_line==data.emp_line){
                ...
                $r_values = data.t_line- data.emp_line;
                $rempo = ($r_values > 1 )?'Templates':'Template';
                $filpo = ($r_values > 1 )?'Files':'File';
                show_faded_popover('#m_t_p_'+order, ''+$r_values+' '+$rempo+' '+$filpo+' Pending ');
            }
