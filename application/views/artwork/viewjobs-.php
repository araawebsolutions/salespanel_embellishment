            ...
            <div class="row artwork-details-row">
                <div class="col-md-4 artwork-details-row-col">
                    ...
                    <ul class="list-none-1">
                        <li><?=$orderinfo['OrderNumber']?>(<?=($orderinfo['site']=="" || $orderinfo['site']=="en")?"en":"fr";?>)</li>

                        <li><?=$counter['roll_jobs']?> (Rolls) + <?=$counter['sheet_jobs']?> (A4)</li>

                        <li><? echo $designer = $this->Artwork_model->get_operator($orderinfo['designer']); if($designer==""){echo '<span style="color:#f3fbfe">.</span>';}?></li>

                        <li><? echo $customercare = $this->Artwork_model->get_operator($orderinfo['assigny']); if($customercare==""){echo '<span style="color:#f3fbfe">.</span>';}?></li>
                 ...
                 <div class="col-md-2 artwork-details-row-col text-center">
                   <? if($counter['roll_jobs']>0 && $this->session->userdata('UserTypeID')==88){?> 
                    <button type="button" class="btn btn-outline-info waves-light waves-effect" style="position: relative;top: 30px;width: 151px;" onClick="window.open('<?=$add_info_url?>')">
                        Add Roll Details
                    </button>
                   <? } ?> 
                 
                  <? if($this->session->userdata('UserTypeID') !=88){?>  
                    <button type="button" onClick="myFunction('send','<?=$orderinfo['OrderNumber']?>');" class="btn btn-outline-info waves-light waves-effect" style="position: relative;top: 40px;">
                        Send to Customer
                    </button>
                  <? } ?>
                </div>
                <div class="col-md-3 artwork-details-row-col">
                 ...
                 ?>  
                  <button type="button" onClick="send_email_again('<?=$orderinfo['OrderNumber']?>');" class="btn btn-outline-info waves-light waves-effect" style="position: relative;top: 40px;">
                        Send Approval Email Again  </button>
                  <? } } ?>      
                </div>
               ...
               <? } ?> 
                <li class="nav-item artwork-detail-li siderbarlist openprintjob" id="active_<?=$job->ID?>" data-id="<?=$job->ID?>">
                    <a aria-expanded="false" class="nav-link nav-link" id="lighter_<?=$job->ID?>">
                       <? if($job->action==0){ $light    = "red-artwork-pulse";} ?>
                       <? if($job->action==1){ $light    = "green-artwork-pulse";} ?>
                       <? if($job->action==2){ $light    = "yellow-artwork-pulse";} ?>  
                       <? if($job->checklist==0){ $light = "blue-artwork-pulse";} ?> 
                       
                        <div class="sk-spinner sk-spinner-pulse <?=$light?>"></div>
                        Print Job No.: <b>PJ<?=$job->ID?></b>
                        <? if($job->variable_data==1){?> <b style="color:red"> - Sequential</b> <? } ?>
                        </a> 
                        <a><b>Name:</b> <?=$job->name?></a><hr>
                    <ul class="artwork-detail-ul">
                         ...
                         <li class="artwork-sub-text-li-subs">Labels<p><b><?=$job->labels?></b></p></li>
                    </ul>
                </li>
               <? } ?> 
            </ul>
...
<script>
    ...
    $(document).on("submit", "#add_softproof", function(e){ 
		 ...
		 if(soft==""){swal('','Attach Softproof','warning'); return false;}
		 if(softproofpdffile==""){swal('','Attach Softproof pdf','warning'); return false;}
		 if(softproofthumbfile==""){swal('','Attach Softproof Thumbnail','warning'); return false;}
		 e.preventDefault();
   ...
   function save_comment(){
		...
		if(comment==""){
		  swal('','Enter comments','warning'); return false;
		}
		...
		$.ajax({
			...
			data:{attachment:attachment,comment:comment,attach:attach,softproof:softproof,thumbnail:thumbnail,pdfformat:pdfformat},
			cache:false,
			dataType: 'json',
			success:function(data){
				if(data.url!=''){
   ...
   $(document).on("submit", "#upload_printfile", function(e){ 
		 ...
		 if(printfile==""){swal('','Attach Print file','warning'); return false;}
		 e.preventDefault();
 ...
 $(document).on("submit", "#uploadnewco", function(e) { 
	 ...
	 function myFunction(type,order){
    	...
    	if(check==0){ swal('','Select Atleast One Print Job','warning'); return false; }else if(check > 0 && type=="send"){
		        $.ajax({
